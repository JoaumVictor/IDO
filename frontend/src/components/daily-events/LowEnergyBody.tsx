...
export function LowEnergyBody({ payload }: { payload: EnergyDeltaPayload }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="neo-pressed-sm rounded-2xl px-5 py-4 flex items-center gap-3">
        <BatteryLow className="w-5 h-5 text-danger shrink-0" />
        <div className="flex flex-col">
          ...
          <span className="text-base font-display font-black text-white">
            {payload.delta} · restou {payload.new_energy}
          </span>
        </div>
      </div>
    </div>
